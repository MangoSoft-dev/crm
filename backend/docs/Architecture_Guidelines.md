...
11. **Nested Entity Resolution**: When resolving nested GraphQL types (e.g. fetching an `Account` within a `User` query), dynamically check the `selectionSetList` for the nested field. If requested, use the respective service (e.g. `AccountService`) to fetch the nested data and map it back to the result object(s) before returning them. To avoid N+1 issues when fetching lists, extract unique foreign keys and resolve them in bulk. Avoid exposing internal entity resolution methods (like `getAccountById`) directly via GraphQL if they are meant just for internal resolution.
